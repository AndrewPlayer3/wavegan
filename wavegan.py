...
def WaveGANGenerator(
  ...
  if train and use_batchnorm:
    update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope=tf.get_variable_scope().name)
    if slice_len == 16384:
...
def WaveGANDiscriminator(
  ...
  if slice_len == 32768:
    ...
    # [64, 1024] -> [16, 2048]
    with tf.compat.v1.variable_scope('downconv_5'):
      output = tf.compat.v1.layers.conv1d(output, dim * 32, kernel_len, 4, padding='SAME')
      output = batchnorm(output)
    output = lrelu(output)
